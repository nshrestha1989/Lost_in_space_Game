...
{
    player_data result;
    bitmap default_bitmap = ship_bitmap(PEGASI);
    ...
    int width = (screen_width() - sprite_width(result.player_sprite)) / 2;

    // Position in the centre of the initial screen
    sprite_set_x(result.player_sprite, width);
    sprite_set_y(result.player_sprite, width);
...
void update_player(player_data &player_to_update)
{
    ...
    player_to_update.fuel_pct = player_to_update.fuel_pct - player_to_update.fuel_pct * 0.000278;
    //   player_to_update.score = player_to_update.score + 1;
}
